...
<div class="max-w-4xl mx-auto p-6 space-y-6">
    ...
    @foreach ($rentals as $rental)
        <div class="bg-white shadow-md rounded-lg p-6 hover:shadow-lg transition-shadow duration-300">
            ...
            <p class="text-gray-600"><span class="font-semibold">User:</span> {{ $rental->user->name }}</p>
            <p class="text-gray-600"><span class="font-semibold">Author:</span> {{ $rental->books->author->name ?? 'N/A' }}</p>
            <p class="text-gray-600"><span class="font-semibold">Genre:</span> {{ $rental->books->genre->name ?? 'N/A' }}</p>
            ...
            <a href="{{ route('rentals.returned') }}"
                class="bg-indigo-900 text-white px-5 py-2 rounded-full text-sm hover:bg-indigo-700 transition">
                Approve
            </a>
        </div>
